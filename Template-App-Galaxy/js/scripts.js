(function ($) {
	...
	$(document).ready(function () {
		...
		$(document).on('click', function(event) { 
			if (!$(event.target).closest('.menu').length && !$(event.target).closest('.menu-open').length && !$(event.target).closest('.submenu-close').length && !$(event.target).closest('.bottom-info').length) {
